...
class Manager
{
    ...
    protected function checkCreateFile($filePath)
    {
        ...
        if (file_exists($filePath)) {
            if (!in_array($this->getPermissionUtils()->getCurrentPermission($filePath),
                array($defaultPermissions['file'], $defaultPermissions['dir']))
            ) {
     ...
     * @param array $content
     *
     * @internal param string $varName - name of variable which contains the content
     * @return string | false
     */
...
?>';
     ...
     * @internal param resource $context
     *
     * @return bool
     */
    ...
    public function copy($sourcePath, $destPath, $recursively = false, array $fileList = null, $copyOnlyFiles = false)
    {
        ...
        if (isset($fileList)) {
            ...
            $fileList = is_file($sourcePath) ? (array)$sourcePath : $this->getFileList($sourcePath, $recursively, '',
                'file', true);
        }
            ...
            if (!in_array($value, array(".", ".."))) {
                ...
                if (is_dir($path . Utils\Util::getSeparator() . $value)) {
                    if ($recursively || (is_int($recursively) && $recursively != 0)) {
                        ...
                        $result[$value] = $this->getFileList($path . Utils\Util::getSeparator() . $value,
                            $nextRecursively, $filter, $fileType);

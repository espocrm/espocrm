...
class Output
{
    ...
    public function processError($message = 'Error', $code = 500, $isPrint = false)
    {
         ...
         */
        $log = $GLOBALS['log'];
        $log->error('API [' . $this->getSlim()->request()->getMethod() . ']:' . $this->getSlim()->router()->getCurrentRoute()->getPattern() . ', Params:' . print_r($this->getSlim()->router()->getCurrentRoute()->getParams(),
                true) . ', InputData: ' . $this->getSlim()->request()->getBody() . ' - ' . $message);
    ...
    public function displayError($text, $statusCode = 500, $isPrint = false)
    {
         ...
         */
        $log = $GLOBALS['log'];
        $log->error('Display Error: ' . $text . ', Code: ' . $statusCode . ' URL: ' . $_SERVER['REQUEST_URI']);
        if (!empty($this->slim)) {
            ...
            $log->info('Could not get Slim instance. It looks like a direct call (bypass API). URL: ' . $_SERVER['REQUEST_URI']);
            die($text);
        }
    }

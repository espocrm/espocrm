...
class Converter
{
    ...
    public function __construct(Manager $fileManager)
    {
        ...
        $this->dbalSchema = new Schema();
        $this->typeList = array_keys(Type::getTypesMap());
    }
    ...
    public function process(array $ormMeta, $entityDefs, $entityList = null)
    {
         ...
         */
        $log = $GLOBALS['log'];
        $log->debug('Schema\Converter - Start: building schema');
        ...
        if (isset($entityList)) {
            ...
            $log->debug('Rebuild Database for entities: [' . implode(', ',
                    $entityList) . '] with dependent entities: [' . implode(', ', $dependentEntities) . ']');
        ...
        foreach ($ormMeta as $entityName => $entityParams) {
            ...
            foreach ($entityParams['fields'] as $fieldName => $fieldParams) {
                if ((isset($fieldParams['notStorable']) && $fieldParams['notStorable']) || in_array($fieldParams['type'],
                        $this->notStorableTypes)
                ) {
                    continue;
                }
                ...
                if (!in_array($fieldType, $this->typeList)) {
                    $log->debug('Converters\Schema::process(): Field type [' . $fieldType . '] does not exist ' . $entityName . ':' . $fieldName);
                    continue;
                }
                $columnName = Util::toUnderScore($fieldName);
                $entityTable = $tables[$entityName];
                if (!$entityTable->hasColumn($columnName)) {
                    $entityTable->addColumn($columnName, $fieldType, $this->getDbFieldParams($fieldParams));
                }
                //add unique
            ...
            $entityTable = $tables[$entityName];
            $entityTable->setPrimaryKey($primaryColumns);
            if (!empty($indexList)) {
                foreach ($indexList as $indexItem) {
                    $entityTable->addIndex($indexItem);
                }
            }
            if (!empty($uniqueColumns)) {
                foreach ($uniqueColumns as $uniqueItem) {
                    $entityTable->addUniqueIndex($uniqueItem);
                }
            }
        }
            ...
            foreach ($entityParams['relations'] as $relationName => $relationParams) {
                switch ($relationParams['type']) {
                    ...
                    case 'belongsTo':
                        ...
                        $entityTable->addIndex(array($columnName));
                        break;
                }
            }
        }
        //END: check and create columns/tables for relations
        $log->debug('Schema\Converter - End: building schema');
        return $schema;
    }
    ...
    protected function prepareManyMany($entityName, $relationParams, $tables)
    {
        ...
        if (isset($relationParams['additionalColumns'])) {
            foreach ($relationParams['additionalColumns'] as $fieldName => $fieldParams) {
                ...
                $table->addColumn(Util::toUnderScore($fieldName), $fieldParams['type'],

...
class EmailNotification extends
    Base
{
    ...
    public function notifyAboutAssignmentJob($data)
    {
        ...
        if ($user && $entity && $assignerUser && $entity->get('assignedUserId') == $userId) {
            ...
            if (!empty($emailAddress)) {
                ...
                $subject = $this->getLanguage()->translate('assignmentEmailNotificationSubject', 'messages',
                    $entity->getEntityName());
                $body = $this->getLanguage()->translate('assignmentEmailNotificationBody', 'messages',
                    $entity->getEntityName());
    ...
    protected function replaceMessageVariables($text, $entity, $user, $assignerUser)
    {
        ...
        $text = str_replace('{entityType}', $this->getLanguage()->translate($entity->getEntityName(), 'scopeNames'),
            $text);

...
class Dialog {
    ...
    constructor(options) {
        ...
        const $body = $("<div>").addClass("modal-body body").html(this.body);
        ...
        const $dialog = $("<div>").addClass("modal-dialog").append($content);
        ...
        if (this.width) {
            ...
            modalContentEl.css(
                "margin-left",
                "-" + parseInt(this.width.replace("px", "")) / 5 + "px"
            );
        }
        ...
        this.$el.on("shown.bs.modal", () => {
            ...
            const headerHeight =
                this.$el.find(".modal-header").outerHeight() || 0;
            const footerHeight =
                this.$el.find(".modal-footer").outerHeight() || 0;
        ...
        if (this.collapseButton) {
            $header.prepend(
                $("<a>")
                    ...
                    .append($("<span />").addClass("fas fa-minus"))
            );
        }
    ...
    getFooter() {
        ...
        const allDdItemsHidden =
            this.dropdownItemList.filter((o) => o && !o.hidden).length === 0;
        ...
        const $dropdown = $("<div>")
            ...
            .append(
                $("<button>")
                    ...
                    .append($("<span>").addClass("fas fa-ellipsis-h"))
            );
    ...
    close() {
        if (!this.onCloseIsCalled) {
            ...
            if (this.activeElement) {
                setTimeout(() => {
                    const element = this._findClosestFocusableElement(
                        this.activeElement
                    );
...
Espo.Ui = {
    ...
    confirm: function (message, o, callback, context) {
        ...
        const processCancel = () => {
            ...
            if (context) {
                o.cancelCallback.call(context, isResolved);

                return;
            }

            o.cancelCallback(isResolved);
        };
    ...
    popover: function (element, o, view) {
        ...
        const $body = $("body");
        const content =
            o.content || Handlebars.Utils.escapeExpression(o.text || "");
        let isShown = false;
    ...
    notify: function (message, type, timeout, options) {
        ...
        const $el = $("<div>")
            ...
            .append($("<div>").addClass("message").html(sanitizedMessage));
        ...
        if (closeButton) {
            ...
            $el.append($("<div>").addClass("close-container").append($close));

    ...
    System
{
    ...
    public function checkRequirements()
    {
        ...
        if (!empty($this->requirements)) {
            if (!empty($this->requirements['phpVersion']) && version_compare(PHP_VERSION,
                    $this->requirements['phpVersion']) == -1
            ) {
    ...
    public function getRecommendationList($type = 'php', $data = null)
    {
        $list = array();
        if ($type == 'mysql') {
            $pdoConn = $this->getPdoConnection($data['dbHostName'], $data['dbPort'], $data['dbUserName'],
                $data['dbUserPass'], $data['dbName']);
        }
        ...
        $recommendations = array_merge($this->requirements[$type . 'Requires'],
            $this->requirements[$type . 'Recommendations']);
    ...
    public function checkDbConnection($hostName, $port, $dbUserName, $dbUserPass, $dbName, $isCreateDatabase = true)
    {
        ...
        if ($result['success']) {
            ...
            if (isset($currentMysqlVersion) && version_compare($currentMysqlVersion,
                    $this->requirements['mysqlVersion']) == -1
            ) {
